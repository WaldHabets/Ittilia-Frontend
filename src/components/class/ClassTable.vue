<template>
    <table>
        <thead>
            <tr>
                <th>Level</th>
                <th><button @click="prev" class="button">
                ...
                <th><button @click="next" class="button">
                    <svg viewBox="0 0 24 24" fill="white">
        ...
        <tbody>
            <tr v-for="headerIndex in table.headers.length" :key="headerIndex">
                ...
                <template v-for="level in levels">
                    <td :key="`generic-class-${level}`" :class="{focus: level === focusLevel}">
            ...
            <template v-if="table.spellsPerLevel != null">
                ...
                <tr v-for="spellLevel in table.spellsPerLevel.max" 
                    :key="`spellLevel-${spellLevel}`" 
                    :class="{hidden: displaySpells}">
                    <td class="row-header">{{ spellLevel }}</td>
                    <td></td>
                    <template v-for="level in levels">
                        <td :key="`spells-class-${level}`" :class="{focus: level === focusLevel}">
...
export default class ClassTable extends Vue {
    ...
    @Prop(Object) private readonly table!: any;

    next(): void {
        if (this.focusLevel === 20)
            return;
